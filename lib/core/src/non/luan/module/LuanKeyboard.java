...
public class LuanKeyboard extends LuanBase {
    ...
    public LuanKeyboard(NonVM vm) {
        super(vm, "NonKeyboard");
    }

    @Override
    public void init() {
        // is_down = non.keyboard.isDown(key1, key2, ...)
        set("isDown", new VarArgFunction() { @Override public Varargs invoke(Varargs args) {
            try {
                boolean isDown = false;

                for (int i = 1; i <= args.narg(); ++i) {
                    isDown |= Gdx.input.isKeyPressed(getKey(getArgString(args, i)));
                }
        ...
        // is_visible = non.keyboard.isVisible()
        set("isVisible", new VarArgFunction() { @Override public Varargs invoke(Varargs args) {
            return valueOf(Gdx.input.isPeripheralAvailable(Peripheral.OnscreenKeyboard));
        }});

        // non.keyboard.setVisible(is_visible)
        set("setVisible", new VarArgFunction() { @Override public Varargs invoke(Varargs args) {
            try {
                Gdx.input.setOnscreenKeyboardVisible(getArgBoolean(args, 1));
            } catch (Exception e) {
                handleError(e);
            } finally {
                return NONE;
